...
print(f"{'Seed':>5}  {'N':>3}  {'Fuel_sav%':>10}  {'Time_sav%':>10}  {'ours_mg':>10}  {'abl_mg':>10}")
print("-"*65)
...
for s in seeds:
    ...
    print(f"{s:>5}  {n:>3}  {fuel_sav:>+10.2f}%  {time_sav:>+10.2f}%  {sum(of)/n:>10.1f}  {sum(af)/n:>10.1f}")

K = len(seed_stats)
if K < 2:
    print(f"\nOnly {K} seeds available"); exit()
...
t_crit = {9:2.262, 8:2.306, 7:2.365, 6:2.447, 5:2.571, 4:2.776, 3:3.182}
...
print(f"\n=== FINAL K={K} seeds (df={K-1}, t_crit={tc:.3f}) ===")
print(f"Fuel saving: {fm:+.2f}%  95%CI=[{fm-tc*fse:+.2f}%, {fm+tc*fse:+.2f}%]  t={ft:+.3f}  d_z={fdz:+.3f}  ours_better={wins_f}/{K}")
print(f"Time saving: {tm:+.2f}%  95%CI=[{tm-tc*tse:+.2f}%, {tm+tc*tse:+.2f}%]  t={tt:+.3f}  d_z={tdz:+.3f}  ours_better={wins_t}/{K}")
